...
"""
Build a final source release package.

Usage: python3.12 release.py
"""
...
def run_logged(cmd, log_file: Path, cwd: Path | None = None) -> subprocess.CompletedProcess:
    with open(log_file, "a") as output:
        return subprocess.run(prepare_subprocess_cmd(cmd), cwd=cwd, stdout=output, stderr=output)


def command_output(cmd, cwd: Path | None = None) -> str:
    try:
        result = subprocess.run(
...
def ensure_python_version() -> None:
    if sys.version_info < PYTHON_MIN_VERSION:
        ...
        print_error(f"Python {required}+ is required. Current version: {platform.python_version()}")
        sys.exit(1)
...
def frontend_deps_need_sync() -> bool:
    ...
    if package_lock.exists() and (
        ...
        or package_lock.stat().st_mtime > package_lock_marker.stat().st_mtime
    ):
        return True
...
def sync_frontend_deps() -> None:
    check_node_runtime()

    if not frontend_deps_need_sync():
        print_info("Frontend dependencies already installed")
        return

    LOG_DIR.mkdir(parents=True, exist_ok=True)
    npm_log_file = LOG_DIR / "npm-release-sync.log"
    ...
    print_kv("Package:", str(FRONTEND_DIR / "package.json"))
    print_kv("Log File:", str(npm_log_file))
    print()

    cmd = ["npm", "ci"] if (FRONTEND_DIR / "package-lock.json").exists() else ["npm", "install"]
