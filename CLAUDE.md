...
- **不要为了兼容写屎代码，宁愿重构也别搞兼容。** 遇到需要向后兼容的场景，优先重构调用方，而非在被调用方堆兼容 shim、fallback 或 deprecated 参数。
- 禁止保留"以防万一"的死代码、未使用的别名、注释掉的旧逻辑。如果确认没用就删干净。
- 重复逻辑超过两处必须提取为共享工具函数或 composable，不允许 copy-paste。
- 类型定义要精确，禁止用 `Any`、`string`、`| string` 等宽泛类型糊弄编译器。
- 导入未验证的外部数据（JSON 导入、API 响应）时必须做 schema 校验，禁止裸 `as` 类型断言。
...
此规范自 2026-04-10 起生效，于 2026-04-21 更新。
