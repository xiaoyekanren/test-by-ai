...
本文档记录当前 TestFlow 与 Node-RED 的主要能力差距，用于后续规划工作流编辑器、执行引擎、节点体系和安全能力。
...
当前执行引擎已经会基于 `edges` 构建 DAG，支持按依赖关系调度、分支并发执行，以及在上游失败时将下游节点标记为 `skipped`。不过它仍然不是 Node-RED 式的消息运行时，`edges` 还没有端口语义、消息载荷或事件驱动能力。
...
- 后端执行器注册表继续向声明式节点注册表收敛，而不是只停留在代码级分发表。
...
| 优先级 | 方向 | 原因 |
|--------|------|------|
| P0 | 统一消息模型与端口语义 | DAG 已落地，但离 Node-RED 式事件流仍差一层抽象 |
