...
`release.py` 生成 `release/仓库-版本/`，并同步生成 `release/仓库-版本.zip`。zip 包内保留同名顶层文件夹，解压后不会把文件散落到当前目录。

版本默认取最近的 Git tag；没有 tag 时回退为 `0.0.0`。需要手动指定版本时，可执行 `python3.12 release.py --version 0.1.0`。
